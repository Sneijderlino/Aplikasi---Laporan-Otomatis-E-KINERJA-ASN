...
def get_current_time_display():
    ...
    # Contoh: 10:55 WIT
    return datetime.now().strftime("%H:%M") + " WIT"

def get_current_time_storage():
    """Mengambil waktu saat ini dan memformatnya sebagai HH:MM (untuk penyimpanan data)."""
    # Contoh: 10:55
...
class LaporanApp:
    def __init__(self, root, username):
        ...
        # [PERUBAHAN WAKTU V1] Gunakan format HH:MM WIT di GUI default
        self.entries["Waktu"].delete(0, "end")
        self.entries["Waktu"].insert(0, get_current_time_display()) 
    ...
    def _write_report_to_pdf_page(self, pdf, row):
        ...
        if os.path.exists(logo_path):
            try:
                pdf.image(logo_path, x=15, y=8, w=16)  # Logo di kiri atas
            except:
                pass
        ...
        # Ambil waktu dan tambahkan " WIT" untuk Tampilan PDF [PERUBAHAN WAKTU V2]
        waktu_val = str(row.get("Waktu", "") or "")
        waktu_display = waktu_val + " WIT" if waktu_val and not waktu_val.upper().endswith(" WIT") else waktu_val
    ...
    # ... (export_all_to_single_pdf dan export_each_row_to_pdf_files tidak berubah)
            ...
            if k == "Tanggal":
                ...
                if val:
                    ...
                    if parsed:
                        ...
                        # Jika input tanggal tidak valid, gunakan tanggal hari ini
                        val = datetime.now().strftime("%d-%m-%Y")
            
            # [PERUBAHAN WAKTU V3] Hapus ' WIT' dari waktu sebelum disimpan
            if k == "Waktu":
                if val.upper().endswith(" WIT"):
                    val = val[:-4].strip() # HH:MM
    ...
    def set_form_data_from_row(self, row):
        ...
        for k in ["Nama","NIP","Jabatan","Unit Kerja","Tanggal","Waktu"]:
            ...
            # [PERUBAHAN WAKTU V4] Tambahkan ' WIT' ke waktu yang di-load dari data Excel
            if k == "Waktu" and val:
                # Cek jika formatnya belum ada ' WIT'
                if not str(val).upper().endswith(" WIT"):
                    val = str(val) + " WIT"
    ...
    def tampilkan_data(self, filtered_df=None):
        ...
        for idx, row in df.reset_index(drop=True).iterrows():
            ...
            for c in COLUMNS:
                ...
                # [PERUBAHAN WAKTU V5] Tambahkan ' WIT' di tampilan tabel
                if c == "Waktu" and v:
                    if not str(v).upper().endswith(" WIT"):
                        v = str(v) + " WIT"
    ...
    def _validate_time(self, s):
        """Memvalidasi format waktu (hanya HH:MM). [PERUBAHAN V6]"""
        if not s:
            return False
        
        # Hapus ' WIT' sebelum validasi format
        s = s.upper().replace(" WIT", "").strip() 
        
        # Hanya cek format HH:MM
        try:
            if len(s) == 5: # HH:MM
    ...
    def _show_data_preview_popup(self, data, action_text, confirm_callback):
        ...
        for col in preview_cols:
            ...
            # [PERUBAHAN WAKTU V7] Tambahkan ' WIT' di pop-up preview
            if col == "Waktu" and val:
                if not str(val).upper().endswith(" WIT"):
                    val = str(val) + " WIT"
    ...
    def tambah_laporan(self):
        ...
        if not self._validate_time(data.get("Waktu","")):
            # Validasi di sini akan menggunakan HH:MM, karena data Waktu di form sudah diubah jadi HH:MM WIT
            messagebox.showwarning("Format waktu", "Format 'Waktu' tidak dikenali. Gunakan HH:MM.")
            return
    ...
    def reset_form(self):
        ...
        # [PERUBAHAN WAKTU V8] Gunakan format HH:MM WIT saat reset
